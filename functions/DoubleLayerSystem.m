classdef DoubleLayerSystem < handle
    ...
    methods
        ...
        function this = DoubleLayerSystem(incidenceEnergy, emissionEnergy, slit, foot, angle, ScatteringFactorFolder)
        ...
        function intensity = getLayerIntensityWithDecayLength(this, decay, angles)
            ...
            lastLayerStartIntensity = intensity(:, end);
            intensity(:, end) = intensity(:, end) .* L .* (term0 + term1 - term2);
            
            % add the decay part
            if size(intensity, 2) > 1
                intensity(:, end-1) = intensity(:, end-1) + lastLayerStartIntensity .* L * decay ./ (L + decay) .* term0;
            end
        ...
        function intensity = calculateSignalCurve(this, P, angles)
            ...
            int = this.getLayerIntensityWithDecayLength(P(3), angles);
            
            % int = this.getLayerIntensity(angles);
            
            conc = P(const+1:end);
            
            intensity = sum(repmat(conc, length(angles), 1) .* int(:, 2:end), 2)' * P(2) + P(4);
...
%             intensity = sum(repmat(conc, length(this.angle), 1) .* this.layerIntensity(:, 2:end), 2)' * P(2) + P(3);
            ...
            int = this.getLayerIntensityWithDecayLength(P(3));
            
            conc = P(const+1:end);
            
            intensity = sum(repmat(conc, length(this.angle), 1) .* int(:, 2:end), 2)' * P(2) + P(4);
            
        end
        
        function intensity = calculateSignalWithDecay(this, P)
        ...
        function intensity = calculateSignalWithBounds(this, start, lower, upper)
            ...
            intensity = this.calculateSignalWithDecay(P);
